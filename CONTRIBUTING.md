...
1. Create a release branch and PR. Target `main` for the current line or the persistent `release/X.Y` branch for a maintained older line. Do not push release changes directly to `main`.
2. Bump the shared version fields listed in `docs/versioning.md`.
3. Regenerate JS artifacts and lockfiles:
   - `pnpm install`
   - `pnpm build`
4. Wait for CI to pass and merge the release PR into its target branch.
5. Switch to the updated target branch, verify `HEAD` matches its remote head, and confirm the worktree is clean.
6. Tag from the target branch and push the tag:
