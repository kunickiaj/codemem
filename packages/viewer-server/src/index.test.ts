...
describe("viewer-server", () => {
	...
	describe("GET /api/sync/peers", () => {
		...
		it("converges local coordinator config when archive is retried after cleanup fails", async () => {
			const configDir = mkdtempSync(join(tmpdir(), "codemem-config-test-"));
			const configPath = join(configDir, "config.json");
			...
			try {
				...
				const { app, cleanup } = createTestApp();
				try {
					const first = await app.request("/api/coordinator/admin/groups/team-a/archive", {
						method: "POST",
					});
					...
					rmSync(lockPath);
					const second = await app.request("/api/coordinator/admin/groups/team-a/archive", {
						method: "POST",
					});
		...
		it("preserves local coordinator config without authoritative archive evidence", async () => {
			const configDir = mkdtempSync(join(tmpdir(), "codemem-config-test-"));
			const configPath = join(configDir, "config.json");
			const prevConfig = process.env.CODEMEM_CONFIG;
			const prevFetch = globalThis.fetch;
			...
			try {
				...
				try {
					for (let attempt = 0; attempt < 6; attempt += 1) {
						const response = await app.request("/api/coordinator/admin/groups/team-a/archive", {
							method: "POST",
						});
						expect(response.status).toBe(400);
						expect(JSON.parse(readFileSync(configPath, "utf8"))).toMatchObject({
