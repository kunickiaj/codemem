...
function propagateHistoricalRepositoryMapping(db: InstanceType<typeof Database>): void {
	...
	const reusedRemoteSession = insertSession(db, {
		cwd,
		gitBranch: null,
		gitRemote: "https://git.example.invalid/exampleco/older-api.git",
		project: "older-api",
	});
	const reusedRemoteMemoryId = insertMemory(db, reusedRemoteSession, {
		originDeviceId: "source-device",
		project: "older-api",
		scopeId: LOCAL_DEFAULT_SCOPE_ID,
	});
