...
function collectKeepReasons(input: {
	...
	// "we should remember", "must remember to ...").
	const personalTask = hasAnyPattern(text, [
	...
	// "must" at end-of-clause or before punctuation doesn't over-match, while
	// still excluding personal-task phrasing above. `requires?` also qualifies.
	const hasModalContract =
		!personalTask &&
		hasAnyPattern(text, [
