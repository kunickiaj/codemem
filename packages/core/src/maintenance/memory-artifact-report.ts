	...
	return withDb(dbPath, (db) => {
		...
		for (const row of rows) {
			...
			const result = classifyMemoryWorthiness({
				kind: row.kind,
				title: row.title,
				// Fold structured facts into the classified text so durable content
				// stored only in `facts` (e.g. after backfill/replication) is not
				// missed by a body-only classification.
				body_text: appendFactsToBody(row.body_text, row.facts),
