...
export function scanSecretsRetroactive(
	...
	const rows = db
		.prepare(
			`SELECT id, rev, title, subtitle, body_text, narrative, tags_text,
			        facts, concepts, metadata_json, actor_display_name, origin_source
			 FROM memory_items
			 ORDER BY id ASC
			 ${limitClause}`,
		)
	...
	const updateStmt = db.prepare(
		`UPDATE memory_items
		 SET title = ?, subtitle = ?, body_text = ?, narrative = ?, tags_text = ?,
		     facts = ?, concepts = ?, metadata_json = ?, actor_display_name = ?,
		     origin_source = ?, updated_at = ?
		 WHERE id = ? AND rev = ?`,
	);
	...
	for (const row of rows) {
		...
		const ok = db.transaction(() => {
			...
			// `memory_concept_refs` derives from `concepts` and indexes the raw
			// concept string. If we redacted a concept on the row, the junction
			// table still holds the unredacted form and acts as a side-channel.
			// Refresh just that table here; `memory_file_refs` derives from
			// files_read/files_modified which this sweep doesn't touch, so we
			// leave it alone.
