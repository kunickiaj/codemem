...
function safeLabel(value: string, fallback: string, forbiddenIds: string[] = []): string {
	...
	if (forbiddenIds.some((id) => id && sanitized.includes(id))) return fallback;
	return sanitized;
}
...
export function refreshLegacyTeamSetupDraftLabels(
	...
	return db.transaction(() => {
		...
		for (const device of input.devices) {
			updateDevice.run(
				safeLabel(device.displayName, "Device", [device.deviceId, ...contextIds]),
				now,
				attemptId,
				device.deviceId,
			);
...
export function setLegacyTeamSetupProjectMapping(
	...
	// recipient edge at activation. Anything that is not itself a canonical
	// workspace identity — a path, alias, or pattern — would grant the Team
	// access to a Project the review never displayed.
	if (
		!identity ||
