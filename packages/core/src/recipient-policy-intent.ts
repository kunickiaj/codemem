...
function projectRecipientIntent(db: Database): RecipientPolicyProjectRecipientV1[] {
	...
	const rows = db
		.prepare(
			`SELECT canonical_project_identity, recipient_kind, recipient_id, status,
			 provenance, policy_revision, updated_at
	...
	for (const value of rows) {
		...
		const persistedProjectIdentity = String(value.canonical_project_identity ?? "");
		const updatedAt = String(value.updated_at ?? "");
		const winner = winnerByKey.get(key);
		const winnerIsCanonical =
			winner?.persistedProjectIdentity === recipient.canonicalProjectIdentity;
		const rowIsCanonical = persistedProjectIdentity === recipient.canonicalProjectIdentity;
		if (
			!current ||
			(!winnerIsCanonical && (rowIsCanonical || updatedAt > (winner?.updatedAt ?? "")))
		) {
