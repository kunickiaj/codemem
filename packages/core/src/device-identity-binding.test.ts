...
it("wakes a canonical authority row for a legacy cwd recipient edge", () => {
	const db = new Database(":memory:");
	try {
		initTestSchema(db);
		const cwd = "/workspace/legacy-checkout";
		const project = "https://git.example.invalid/acme/legacy-checkout.git";
		...
		).run(project, NOW, NOW, NOW, NOW);

		wakeRecipientPoliciesForProjectIdentities(db, [cwd], NOW);
...
it("does not scan repository history for an empty policy wake", () => {
	...
	try {
		...
		wakeRecipientPoliciesForProjectIdentities(db, [], NOW);
