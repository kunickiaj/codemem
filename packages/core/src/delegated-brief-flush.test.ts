...
describe("current-batch delegated brief extraction", () => {
	it("bounds a current-batch brief without displacing tool evidence", async () => {
		...
		const session = store.db
			.prepare("SELECT metadata_json FROM sessions ORDER BY id DESC")
			.get() as {
			metadata_json: string;
		};
		const metadata = JSON.parse(session.metadata_json) as {
			session_context: { durationMs?: number };
		};
		expect(metadata.session_context.durationMs).toBe(600_000);
	});
});
