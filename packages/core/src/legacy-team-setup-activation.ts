...
export interface FinishLegacyTeamSetupActivationInput
	 ...
	 * discovery uses). Displayed evidence that changed after preview — for
	 * example a newly ingested session adding a Project — must reject the
	 * finish rather than commit a completion discovery will immediately
	 * replace; the persisted draft rows alone cannot prove the review is
	 * still complete.
	 */
	loadProjectInventory: () => Promise<LegacyTeamSetupProjectInput[]>;
	now?: string;
}
...
function loadModel(db: Database, input: PreviewLegacyTeamSetupActivationInput): ActivationModel {
	...
	const mappings = db
		.prepare(
			`SELECT id, workspace_identity, project_pattern, scope_id
...
function validateCanonicalState(model: ActivationModel): void {
	const { memberships, projects, recipients, scopeIds, team, teamId } = model;
	if (team) {
		...
		if (
			relatedMappings.some(
				(mapping) =>
					...
					mapping.workspace_identity !== resolvedIdentity,
			)
		) {
			activationError("team_setup_conflict");
		}
		// Creating a brand-new mapping requires an unambiguous target scope.
		if (relatedMappings.length === 0 && scopeIds.length > 1) {
			activationError("team_setup_conflict");
		}
...
function simulatedDerivationRows(model: ActivationModel): DerivationRows {
	...
	for (const row of model.allDecisions) {
		if (
			row.team_id === model.teamId &&
			!["team_invite", "coordinator_invite"].includes(row.provenance) &&
			!draftDeviceIds.has(row.device_id)
		) {
	...
	for (const device of model.devices) {
		...
		if (device.decision === "removed") {
			// Mirrors the apply-side removed reconcile: invite-owned decisions
			// survive a reviewed removal.
	...
	for (const project of model.projects) {
		...
		db.prepare(
			`INSERT INTO project_recipients(
			 ...
			 status = 'active', provenance = excluded.provenance,
			 policy_revision = excluded.policy_revision, migration_state = 'completed',
...
export async function finishLegacyTeamSetupActivation(
	...
	let liveProjectionFingerprint: string;
	try {
		freshRoster = await input.loadFreshRoster();
		liveProjectionFingerprint = legacyTeamProjectionFingerprint(await input.loadProjectInventory());
	} catch {
	...
	try {
		return db
			.transaction(() => {
				...
				// saw; the locked draft's fingerprint must match the live one.
