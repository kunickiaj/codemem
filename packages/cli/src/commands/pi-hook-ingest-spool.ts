...
export class PiHookLockBusyError extends Error {
	constructor() {
		super("pi-hook-ingest lock busy");
		this.name = "PiHookLockBusyError";
	}
}
...
function lockConfig(): LockConfig {
	return {
		lockDir: expandHome(
			process.env.CODEMEM_PI_HOOK_LOCK_DIR?.trim() || "~/.codemem/pi-hook-ingest.lock",
		),
		ttlSeconds: Math.max(1, envInt("CODEMEM_PI_HOOK_LOCK_TTL_S", DEFAULT_LOCK_TTL_S)),
		graceSeconds: Math.max(1, envInt("CODEMEM_PI_HOOK_LOCK_GRACE_S", DEFAULT_LOCK_GRACE_S)),
	};
}

export function piHookSpoolDir(): string {
	return expandHome(process.env.CODEMEM_PI_HOOK_SPOOL_DIR?.trim() || "~/.codemem/pi-hook-spool");
}

export function piHookLockTtlSeconds(): number {
	return lockConfig().ttlSeconds;
}

export function hasPiHookSpooledEntries(): boolean {
	let entries: string[];
	try {
		entries = readdirSync(piHookSpoolDir());
	} catch {
		return false;
	}
	return entries.some(
...
export async function withPiHookIngestLock<T>(fn: () => Promise<T> | T): Promise<T> {
	...
	if (!acquired) throw new PiHookLockBusyError();
	try {
		return await fn();
	} finally {
...
export function spoolPiHookPayload(payload: Record<string, unknown>): boolean {
	const dir = piHookSpoolDir();
	try {
		mkdirSync(dir, { recursive: true });
	} catch {
		logHookEvent("codemem pi-hook-ingest failed to create spool dir");
		return false;
	}
	...
	try {
		...
		logHookEvent("codemem pi-hook-ingest failed to allocate spool temp file");
		return false;
	}
	...
	try {
		...
		logHookEvent("codemem pi-hook-ingest failed to spool payload");
		return false;
	}
	logHookEvent(`codemem pi-hook-ingest spooled payload: ${finalPath}`);
	return true;
}

export function recoverStalePiHookTmpSpool(ttlSeconds: number): void {
	const dir = piHookSpoolDir();
	try {
		mkdirSync(dir, { recursive: true });
	} catch {
...
export type PiSpoolHandler = (payload: Record<string, unknown>) => Promise<boolean> | boolean;

export async function drainPiHookSpool(
	handler: PiSpoolHandler,
