...
export function buildCoordinatorCommand(): Command {
	...
	const coordServeCmd = new Command("serve")
		...
		.option("--coordinator-host <host>", "bind host", "127.0.0.1")
		.option("--coordinator-port <port>", "bind port", "7347");
	...
	// Hidden host/port aliases for backwards compat (defaults match canonical options)
	coordServeCmd.addOption(new Option("--host <host>", "bind host").default("127.0.0.1").hideHelp());
	coordServeCmd.addOption(new Option("--port <port>", "bind port").default("7347").hideHelp());
