...
export const createDelegationContext = ({
  ...
  const observe = (event) => {
    ...
    if (!validId(metadata?.sessionId) || metadata.parentSessionId !== part.sessionID
      ...
      || typeof input.prompt !== "string" || !input.prompt || input.prompt.length > MAX_BRIEF_CHARS
      || typeof input.description !== "string" || !Number.isFinite(state.time?.start)) {
      if (previous) previous.ambiguous = true;
      return;
    }
    const fingerprint = digest(JSON.stringify([metadata.sessionId, input.subagent_type, input.prompt, state.time.start]));
    if (previous) {
    ...
    bindings.set(key, {
      ...
      agent: input.subagent_type, brief: input.prompt, startedAt: state.time.start,
      observedAt: now(), messageID: null, ambiguous: false,
    });
  };
  const matchSnapshot = (snapshot, { sessionID, messageID, text }) => {
    ...
    if (part?.type !== "text" || part.synthetic || part.ignored || typeof part.text !== "string"
      || part.sessionID !== sessionID || part.messageID !== messageID || part.text !== text) return null;
    prune();
    const matches = [...bindings.values()].filter((binding) => !binding.closed
      && binding.child === sessionID && binding.parent === session.parentID
      && binding.agent === info.agent && binding.brief === part.text
    ...
    if (requirePromptSnapshot) {
      ...
      if (draft?.message?.id !== messageID || draft.message.sessionID !== sessionID
        ...
        || draftPart.synthetic || draftPart.ignored || draftPart.text !== text) return null;
    }
  ...
  const observePrompt = (input, output) => {
    ...
    for (const binding of bindings.values()) {
      ...
      if (output.parts?.length === 1 && part?.type === "text" && !part.synthetic && !part.ignored
        && part.text === binding.brief) binding.prompt = { message: info, parts: output.parts };
    }
  };
  const resolve = (prompt) => {
    const { sessionID, messageID } = prompt;
    if (disposed || !validId(sessionID) || !validId(messageID)) return Promise.resolve(null);
    prune();
    ...
    if (existing) return existing.text === prompt.text ? existing.promise : Promise.resolve(null);
    ...
    activeLookups.set(key, { text: prompt.text, promise: completion.promise, cancel: () => finish(null) });
    void Promise.all([
      ...
      if (!finished) finish(matchSnapshot(snapshot, prompt));
    }).catch(() => finish(null));
