...
export const createDelegationContext = ({
  ...
  const observePrompt = (input, output) => {
    ...
    for (const binding of bindings.values()) {
      ...
      const promptMatches = output.parts?.length === 1 && part?.type === "text"
        && !part.synthetic && !part.ignored
        && normalizedPromptText(part.text) === binding.brief;
      if (!requirePromptSnapshot || promptMatches) binding.messageID = info.id;
      if (promptMatches) {
        binding.prompt = { message: info, parts: output.parts };
      }
    }
  };
  const boundChildren = (parentSessionID) => {
    if (disposed || !validId(parentSessionID)) return [];
    prune();
    return [...new Set([...bindings.values()]
      .filter((binding) => !binding.closed && binding.parent === parentSessionID)
      .map((binding) => binding.child))];
  };
  const boundChild = (parentSessionID, callID) => {
    if (disposed || !validId(parentSessionID) || !validId(callID)) return null;
    prune();
    const binding = bindings.get(JSON.stringify([parentSessionID, callID]));
    if (binding?.closed || binding?.parent !== parentSessionID || !validId(binding?.child)) return null;
  ...
  return { boundChild, boundChildren, observe, observePrompt, resolve, dispose: () => {
    disposed = true;
