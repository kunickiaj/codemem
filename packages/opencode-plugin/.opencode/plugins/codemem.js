...
const dispatchV1Event = async (delegation, runtime, event) => {
  const translated = translateV1Event(event);
  if (event?.type === "session.deleted") {
    try {
      for (const childSessionID of delegation.boundChildren(translated.sessionID)) {
        await runtime.capturePendingPrompts({ sessionID: childSessionID });
      }
      return await runtime.handleEvent(translated);
    } finally {
      delegation.observe(event);
    }
  }
  const part = event?.properties?.part;
  const terminalTaskUpdate = event?.type === "message.part.updated"
    && part?.type === "tool"
    && part.tool === "task"
    && ["completed", "error"].includes(part.state?.status);
  const boundChildSessionID = terminalTaskUpdate
    ? delegation.boundChild(part.sessionID, part.callID)
    : null;
  if (boundChildSessionID) {
    try {
      await runtime.capturePendingPrompts({ sessionID: boundChildSessionID });
      return await runtime.handleEvent(translated);
    } finally {
      delegation.observe(event);
    }
  ...
  delegation.observe(event);
  return runtime.handleEvent(translated);
};
...
const createOpenCodeV1Adapter = async ({ project, client, directory, worktree }) => {
  ...
  return {
    ...
    event: ({ event }) => dispatchV1Event(delegation, runtime, event),
