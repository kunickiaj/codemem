...
describe("OpenCode task-specific capture bindings", () => {
	...
	it("matches edge-whitespace after runtime prompt normalization", async () => {
		const spacedText = ` \n${text}\t `;
		const taskEvent = task();
		taskEvent.properties.part.state.input.prompt = spacedText;
		const snapshot = message();
		snapshot.parts[0].text = spacedText;
		const binding = fixture({
			requirePromptSnapshot: true,
			readMessage: async () => snapshot,
		});
		binding.observe(taskEvent);
		binding.observePrompt(
			{ sessionID: "child", messageID: "msg-1", agent: "explore" },
			{ message: snapshot.info, parts: snapshot.parts },
		);

		expect(await binding.resolve(prompt)).toMatchObject({
			origin: "delegated_brief",
			brief_sha256: createHash("sha256").update(text).digest("hex"),
		});
	});

	it("reserves distinct V1 prompt snapshots only after their briefs match", async () => {
		const firstText = text;
		const secondText = "Review the retry fix and report only verified regressions.";
		...
		const binding = fixture({
			requirePromptSnapshot: true,
			readMessage: async (_session, id) => snapshots.get(id),
		});
		const secondTask = task("call-2");
		secondTask.properties.part.state.input.prompt = secondText;
		binding.observe(task());
		binding.observe(secondTask);
		for (const snapshot of snapshots.values()) {
			binding.observePrompt(
				{
					sessionID: "child",
					messageID: snapshot.info.id,
					agent: "explore",
				},
				{ message: snapshot.info, parts: snapshot.parts },
			);
		...
		await expect(binding.resolve({ ...prompt, text: firstText })).resolves.toMatchObject({
			task_call_id: "call-1",
			message_id: "msg-1",
		});
		await expect(
			binding.resolve({ sessionID: "child", messageID: "msg-2", text: secondText }),
		).resolves.toMatchObject({ task_call_id: "call-2", message_id: "msg-2" });
	});
...
describe("delegation binding task completion", () => {
	it("honors a reserved lookup when task completion closes its binding", async () => {
