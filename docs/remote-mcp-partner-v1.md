...
```
[Adam Claude] ── OAuth ──► [Adam MCP endpoint]   ──►  [Adam codemem peer]
                            (Adam's OIDC subject)        │
                                                         │  Sharing domain
                                                         │  `home-shared`
                                                         │  (explicitly granted
                                                         │   to both peers)
                                                         │
[Partner Claude] ─ OAuth ─► [Partner MCP endpoint] ──►  [Partner codemem peer]
                            (Partner's OIDC subject)
```

Each side is a complete Phase 1 deployment:

- Own host (Pi4, VPS, or laptop).
...
Pick a stable, human-readable name (`home-shared`, `partner-research`, etc.). Create the domain on host A and grant it to host B's device key. The viewer UI under Settings → Device Sync exposes Sharing-domain management; the equivalent CLI lives under `codemem sync` and `codemem coordinator` (see [user-guide.md](./user-guide.md#peer-to-peer-sync)).
...
- **No partner OAuth login to the owner's MCP endpoint.** Each endpoint stays single-principal. Anthropic-facing OAuth tokens issued by Adam's endpoint are bound to Adam's identity only.
...
- **No partner-tagged writes.** A memory written through Adam's endpoint into the shared domain is locally owned by Adam and propagates to Partner; there is no per-principal write attribution beyond device-level metadata.
