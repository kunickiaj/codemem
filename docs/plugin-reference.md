...
- **Payloads are accumulating in a hook spool.** The Viewer is unavailable, rejected the configured target, or could not durably queue the event. Current Claude and Codex compatibility commands do not open SQLite for ordinary transport failures; they retain the native payload and drain entries in order through Viewer HTTP after recovery. Claude terminal boundary events keep direct SQLite ingest and flush as the final safeguard.
