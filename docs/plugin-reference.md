...
`--base` is required. Use `--base auto` to prefer an available local remote-default ref (including renamed remotes) and fall back to `HEAD`; the pre-commit hook uses this mode so commits do not depend on `origin/main`. Pass `--head` for a committed ref-to-ref comparison, or pass `--staged` to compare only the Git index as the pre-commit hook does; those flags cannot be combined. Omit both to freeze the current working tree, including untracked files that are not ignored. The command materializes detached temporary worktrees and never stashes, resets, or checks out over the active worktree. It runs the pinned Biome binary with the head snapshot's policy against both sides, then compares diagnostics only for added, modified, renamed, or deleted paths.
