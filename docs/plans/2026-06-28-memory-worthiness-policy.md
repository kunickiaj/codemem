...
Derived facts preserve durable knowledge extracted from a session, review, debug
...
Derived-fact generation must be idempotent.

Rules:

- rerunning extraction should not create duplicate facts
- derived facts should carry enough source/version metadata to detect repeats
- changing extractor versions should be auditable
- derivation should be reversible by ignoring derived output, not by deleting
  source summaries
