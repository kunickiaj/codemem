...
6. Let cached custom models use an explicit commit identity without a network lookup when remote access is disabled.
...
- A shared validator checks the embedding package, model, revision shape, dtype, device, dimensions, and consistency between client fields and identity fields.
...
- Later stale-model deletion batches reuse that phase only while the database mutation fence remains unchanged; concurrent changes restart validation before completion.

This keeps shutdown work bounded without weakening cutover checks.

## Offline Commit Pins

A lowercase 40-character hexadecimal revision is accepted as an explicit commit identity before cache lookup when remote access is disabled. This permits offline use of a cached custom commit, while other revision forms retain the existing fail-closed behavior.
