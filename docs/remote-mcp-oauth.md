...
[Anthropic backend] ──── HTTPS w/ bearer ────►
                                              │
[https://codemem-mcp.<tailnet>.ts.net/mcp]    │
       ▲                                      │
       │ Tailscale Funnel (public ingress)    │
       │                                      ▼
[Pi4 or other host: codemem mcp http]
...
The viewer process and `codemem mcp http` process must run on different ports. Only the MCP port is exposed via Funnel.
...
- A Tailscale tailnet with Funnel enabled, or another HTTPS-terminating public ingress that preserves the configured Host header.
...
|----------|----------|---------|
| `CODEMEM_MCP_HTTP_PUBLIC_URL` | yes | Externally reachable `/mcp` URL, e.g. `https://codemem-mcp.<tailnet>.ts.net/mcp`. Used in OAuth metadata, code redirects, and the Origin gate. |
...
```fish
set -x CODEMEM_MCP_HTTP_PUBLIC_URL https://codemem-mcp.<tailnet>.ts.net/mcp
...
Behind Tailscale Funnel (recommended):

```sh
sudo tailscale funnel --bg 38889
```

The Funnel will terminate TLS and forward the public hostname/port to your local loopback. The Host header arriving at `codemem mcp http` will match the configured public URL hostname, which is what the bearer-gated `/mcp` route checks.
...
2. Add `<MCP base URL>/oauth/callback` as an authorized redirect URI, e.g.
   `https://codemem-mcp.<tailnet>.ts.net/oauth/callback`.
...
2. Paste the MCP URL: `https://codemem-mcp.<tailnet>.ts.net/mcp`.
...
```sh
curl -X POST "https://codemem-mcp.<tailnet>.ts.net/oauth/revoke" \
...
|---------|--------------|---------------|
| Claude shows "couldn't reach connector" | Funnel not running, or hostname mismatch | `sudo tailscale funnel status`; confirm public URL hostname matches `CODEMEM_MCP_HTTP_PUBLIC_URL`. |
...
| `POST /mcp` returns 403 with valid bearer | Host/Origin mismatch with `CODEMEM_MCP_HTTP_PUBLIC_URL` | Confirm Funnel preserves the configured public hostname. Audit log will not record this; the request is rejected before bearer verification. |
...
Before announcing the connector ready, run the steps below against your live Pi4. The same code paths are covered by `pnpm exec vitest run packages/mcp-server/src/http.test.ts packages/mcp-server/src/oauth.test.ts packages/mcp-server/src/oidc.test.ts` for unit-level regression. The checklist below confirms the live deployment is wired correctly end-to-end.
