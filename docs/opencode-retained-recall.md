...
Assembly keeps retrieval order within the existing recap policy groups, task recency/summary filters, and summary/timeline/observation sections. Retrieved observations are no longer re-sorted by kind, tag overlap, or recency. Manual timeline recap expansion and browsing-only supplemental observation ordering remain supported; automatic explicit recap does not add neighboring timeline records that retrieval did not select.
