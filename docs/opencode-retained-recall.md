...
| Field | Definition |
|---|---|
| `freshEvaluations` | First accepted measurement per session/message/surface/query/artifact evaluation key in the retained ledger. Without stable host session/message IDs, a plugin-instance discriminator, prompt counter, and message position distinguish turns. Transport retries and repeated empty-result transforms within the same turn do not increase this denominator; changed retrieval artifacts do. Replay and reconstruction do not create measurements. |
