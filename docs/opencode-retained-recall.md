...
2.0.2 runs automatic recall through `session.context` only: it requires the
latest user-message ID and skips safely when that identity is missing. Retries
and tool continuations replay retained context byte-for-byte; compaction, title,
and generate hooks stay isolated.
...
Core preserves a named topic through retrieval and allows automatic topical misses to remain empty.

Task mode recognizes bare collection labels such as `backlog`, `pending tasks`, or `follow-ups`, and explicit `show`/`list` requests followed by those collections. Supported prefixes are `please`, `me`/`us`, and the collection modifiers `my`, `our`, `the`, `pending`, and `open`; topics follow `for`, `about`, or `in`, as in `show me pending tasks for Orchid`. A technical clause such as `show how the scheduler queues tasks` uses normal retrieval. Continuation verbs alone do not expand the query into task hints, even for a bare `continue` or `what's next`; use an explicit backlog request to browse tasks. This routing applies to manual packs too, which retain recent-memory fallback when retrieval returns nothing.

Hook context does not turn a listing request into an unrecognized sentence. For task intent only, Core removes an exact trailing suffix reconstructed as the supplied project label followed by the last five working-set file basenames, matching the OpenCode and Claude query builders. Thus `show pending tasks codemem pack.ts` can classify as browsing while `show the pending tasks table schema codemem pack.ts` cannot. The full query still reaches retrieval, including all prompt, project, and file text; task mode retains its existing hint expansion. Query identity is unchanged. Missing/mismatched metadata or a suffix cut by the builders' 500-character cap leaves the query unstripped; Core does not try partial suffixes or separate combined first/latest prompts.

Assembly keeps retrieval order within the existing recap policy groups, task recency/summary filters, and summary/timeline/observation sections. Retrieved observations are no longer re-sorted by kind, tag overlap, or recency. Manual timeline recap expansion and browsing-only supplemental observation ordering remain supported; all automatic requests suppress timeline neighbor expansion, including topical recall such as `What did we decide last time about quartz?`.

Automatic non-task requests reject the entire semantic-only batch when scoped, eligible keyword retrieval has no support, including after a topical recall retry. Nearest-neighbor search has no calibrated confidence cutoff, so the guard adds no distance threshold and deliberately rejects useful paraphrases too; explicit/manual semantic retrieval and genuine task browsing retain those candidates. When keyword support exists, semantic-only items may still join the hybrid batch without matching a keyword result individually; direct working-set file-reference retrieval also remains supported independently.

Dropped eligible semantic candidates carry the trace reason `automatic_semantic_only_without_keyword_support`. Candidates admitted independently through a file reference or the existing requester-summary fallback are not reported as rejected. Project, visibility, and requester gates still run before candidates reach the trace.

Pack source metrics count admitted retrieval candidates, not necessarily final rendered items. A rejected semantic-only batch contributes zero to `response.metrics.sources.semantic` while its eligible candidates and rejection reasons remain in the trace; admitted candidates can still count when later budgeting removes them. The semantic source count retains its existing exclusion of IDs already returned by keyword retrieval.

Requests carrying automatic context permit unrelated recent-memory fallback only for explicit task browsing. Explicit recap requests recognized by the existing `queryPrefersRecap` policy, including `recap of the sprint` and `what happened yesterday`, may fall back to the requester's own latest summary under the existing project/access gates. This fallback adds no recent durable observations and cannot use a foreign summary; without a mapped requester or an eligible own summary, a retrieval miss remains empty. It does not verify that the fallback summary covers the requested topic or time period. Manual recap keeps its existing browsing behavior, and directly retrieved durable facts remain eligible.

Other automatic queries skip recent, summary-hint, latest-summary, and supplemental-observation fallback, including when a small topical result leaves unused space. Empty selections produce no new plugin block; retained conversation context remains available.

A bare `Continue` therefore returns no automatic items when ordinary retrieval finds no eligible match. This follows from the unchanged query and disabled fallback, not a continuation phrase denylist: an eligible matching memory can still be retrieved. The current Core and Viewer contracts use the unchanged historical fixture to check both the empty automatic result and successful explicit Quartz retrieval under the same project and requester-session gates; the immutable historical gold and runner continue to describe the earlier policy. Manual packs retain browsing fallback.

Limits: this is lexical intent handling, not a topic classifier. `Show pending tasks` could mean application data rather than stored work; the listing grammar cannot distinguish those meanings. Other browsing phrasings, such as `display tasks`, `what tasks remain`, or `list Orchid tasks`, keep ordinary retrieval instead of guessed expansion. The existing recall tokenizer can treat ordinary filler as topic tokens. Shared words such as `next` can still retrieve weak lexical matches; preserving rank does not reject every off-topic candidate. Recap fallback requires recognizable recap wording, and a topic-specific recap can still retrieve a summary through shared recap terms. Session/project/access rules, compression, and token budgets are unchanged.

## Local Measurements

Health's collapsed **Automatic recall (advanced)** panel summarizes durable local fresh-evaluation measurements without copying sensitive content. Zero-item packs preserve the backend artifact fingerprint for evaluation identity even though their rendered headings are suppressed; unchanged-artifact retries count once, while changed artifacts count separately. Structured `automatic_recall_write_failed` responses are terminal ledger rejections, not transport failures: they do not trigger CLI fallback or Viewer backoff.
