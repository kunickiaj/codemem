...
- or adopt the exact legacy row previously materialized from this candidate by `choose_recipients` when its deterministic Team ID, `reviewed_team_candidate` provenance, source fingerprint, and memberships match, and its complete recipient-edge set equals the aggregate edges produced by every matching saved resolution across all Projects. Preserve that aggregate edge set, expose its complete effective-access delta in review, and convert the Team to `reviewed_allowlist` only in the confirmed finish transaction. Any edge not justified by a matching resolution, any missing expected edge, or any other mismatched canonical state returns `team_setup_conflict`;
...
0. Add an immutable completion table with `UNIQUE (attempt_id, finish_digest)`, storing the attempt's immutable candidate reference, confirmed access-delta digest, completed Team ID, completion timestamp, and exact bounded response. This constraint backs the concurrent-finish loser replay path, while the stored candidate reference and confirmation digest bind every replay to its candidate route and exact confirmed representation.
...
- Finish performs no coordinator or other network I/O while holding the SQLite write lock.
...
- A completed retry with a missing or mismatched `confirmed_access_delta_digest` returns `team_setup_confirmation_stale` rather than replaying success.
