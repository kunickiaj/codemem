...
Before confirmation, Codemem fetches and validates a fresh roster.
...
Finish validates the opaque candidate route reference, then checks for that completion both before activation and again after acquiring the `BEGIN IMMEDIATE` write lock. Both lookups require the completion's immutable candidate reference to match the route candidate. Concurrent identical requests therefore serialize onto one immutable record: the first request writes it, while the second replays the exact stored response before validating changed canonical state. If insertion encounters a completion-key uniqueness race, the loser loads and returns the committed winner only after verifying the same candidate binding. Completion tokens submitted through another candidate route never replay success.

The finish key is `(attempt_id, finish_digest)`. The immutable completion also retains the attempt's candidate reference, and replay lookup uses `(candidate_ref, attempt_id, finish_digest)` so route validation does not depend on trusting digest contents alone. The digest covers the attempt ID, candidate reference, fresh roster fingerprint, normalized decisions and assignments, expected assignment versions, the complete validated canonical Team snapshot, and the confirmed complete access-delta digest. Therefore a later setup attempt cannot collide with an earlier completion even when its visible roster and decisions are identical, and another candidate route cannot replay the stored result.
...
Historical versions could materialize the deterministic Team and Project recipient edges through `choose_recipients` before guided review. Guided setup may adopt only the exact row tied to the same saved candidate resolution when deterministic identity, `reviewed_team_candidate` provenance, source fingerprint, memberships, and all recipient edges validate. It preserves those edges and reviews their effective access before converting the Team to `reviewed_allowlist`; any extra, missing, or mismatched state fails with `team_setup_conflict`.
...
- compatible `choose_recipients` Teams are adopted without dropping recipient edges, while mismatched legacy rows fail closed;
