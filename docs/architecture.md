...
The OpenCode 1 plugin injects a memory pack automatically on every turn. Volatile recall output is appended beside the latest user message by default so provider prompt caches can keep the stable system/history prefix. The OpenCode 2 entrypoint (beta integration, validated on 2.0.12) uses `session.context` only: it requires a non-empty, non-whitespace latest user-message ID, skips safely when that identity is absent or blank, and performs one fresh retrieval per identified turn. Retries and tool continuations replay retained context byte-for-byte; compaction, title, and generate hooks do not receive automatic recall. Both the default message surface and legacy system surface are supported.
